...
const EditTaskScreen = ({navigation, route}) => {
  ...
  useEffect(() => {
    ...
    if (task.target_completion_time) {
      const date = new Date(task.target_completion_time.replace(' ', 'T'));
  ...
  const formatDateTimeForDb = () => {
    ...
    return `${targetDate} ${targetTime}:00`;
  };
  ...
  const handleUpdate = async () => {
    ...
    const result = await supabaseService.updateTask(token, task.id, taskData);
        ...
        <View style={styles.inputContainer}>
          ...
          <TouchableOpacity
            ...
            <Text style={styles.pickerButtonText}>
              {statuses.find(s => s.id === statusId)?.description || 'Select status'}
            </Text>
            ...
            <View style={styles.pickerOptions}>
              {statuses.map(status => (
                <TouchableOpacity
                  ...
                  style={[
                    styles.pickerOption,
                    statusId === status.id && styles.pickerOptionSelected,
                  ]}
                  onPress={() => {
                  ...
                  <Text
                    style={[
                      ...
                      statusId === status.id && styles.pickerOptionTextSelected,
                    ]}>
                    {status.description}
                  </Text>
                </TouchableOpacity>
